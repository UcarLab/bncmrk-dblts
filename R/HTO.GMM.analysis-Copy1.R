...
library(mclust)
#rm(list = ls())
setwd("~/Codes/ToyModels")
#setwd("/Users/lawlon/Documents/CZI/CITE-seq/PBMC_2018/Pooled_Runs/HTO_Analysis/")
name <- "CZI.PMBC.HTO.pooled"

all_hto <- readRDS("CZI.PBMC.HTO.ordered.count.matrix.Rds")
  
# CLR normalize the data for GMM modeling
sr_obj <- CreateSeuratObject(counts = all_hto[c("control-GTCAACTCTTTAGCG", "Mono-AGTAAGTTCAGCGTA", 
...
for(i in 1:length(c.sum)) {
  ...
  if(c.sum[i] > 5 & length(which(p.bin[,i] ==2)) == 2) {
    s.type[i]="DBL"
    tr.s.type[i]="DBL"
  } else if (c.sum[i] > 5 & length(which(p.bin[,i] ==2)) == 3){
    s.type[i]="DBL"
    tr.s.type[i]="TRP"
  } else if (c.sum[i] > 5 & length(which(p.bin[,i] ==2)) == 4){
    s.type[i]="DBL"
    tr.s.type[i]="QDR"
  }
...
write.csv(x = table(s_anns$Run, s_anns$Sample_Short), file = paste(name, "sample.classifications.GMM_.summary.csv", sep = "."))
write.csv(x = per_s, file = paste(name, "sample.classifications.GMM_.summary.proportions.csv", sep = "."))
write.csv(x = s_anns, file = paste(name, "sample.classifications.GMM_.csv", sep = "."),row.names=FALSE)
